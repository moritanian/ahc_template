...
8. relative score が改善した場合は Git commit を行う。
   - commit message は
     `<内容> (N=<N>) : <local 実行の relative score (%)>, <local 実行の Avg Score>`
     の形式で、**N を明記し、relative score と Avg Score の両方を残す**。
   - 例: `Apply penalty for xxx (N=100) : 92.04%, 4943`
   - 例: `Tune beam width (N=1000) : 85.83%, 5012`
...
commit message は
`<内容> (N=<N>) : <local N ケース実行の relative score (%)>, <local N ケース実行の Avg Score>`
の形式で、**N (評価ケース数) を明記し、relative score と Avg Score の両方を残す**こと。
...
Avg Score は `phst results list` の `Avg Score` 列、または実行結果の
`Average Score` をそのまま使う。

ドキュメントのみの修正（例: `AGENTS.md`
などコードへ影響しない変更）のコミットでは relative score / Avg Score
